...
test('npm release validation ties the tag/repository to the manifest and keeps prereleases off latest', () => {
  assert.deepEqual(validateReleaseMetadata(manifest, context), { version: '0.1.0-alpha.0', distTag: 'latest' });
  assert.equal(validateReleaseMetadata({ ...manifest, version: '1.0.0' }, { ...context, ref: 'refs/tags/v1.0.0' }).distTag, 'latest');
  assert.equal(validateReleaseMetadata({ ...manifest, version: '1.0.0-release-candidate.0' }, { ...context, ref: 'refs/tags/v1.0.0-release-candidate.0' }).distTag, 'latest');
