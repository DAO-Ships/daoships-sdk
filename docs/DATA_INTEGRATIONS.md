...
Profile reads use canonical record order by default: `new DaoShipsData(indexer)` and `connectDaoShipsSupabase` both request event positions. The hosted mainnet and testnet schemas carry the migration and a complete receipt-verified backfill (2026-09-23). Pass `{ recordOrdering: false }` only for a self-hosted indexer without the migration. `indexer.listOrderedRecords()` exposes the additive `transaction_index` and `log_index` columns using canonical block/transaction/log ordering. Both coordinates come from actual Quai logs; the migration leaves historical values NULL until receipt-verified backfill. Unknown positions sort first within their block, so a known new record cannot conceal an unordered legacy record. Distinct, known positions resolve same-block profiles; mixed/unknown positions or duplicate coordinates remain ambiguous. Ordered reads against an unmigrated schema fail visibly rather than guessing; opt out there. `recordOrdering: false` restores the baseline projection, which never requests these columns. See the upstream rollout and backfill procedure in the indexer repository’s `docs/RECORD_EVENT_ORDER.md`; preparation is not evidence that a hosted migration or backfill has run.
